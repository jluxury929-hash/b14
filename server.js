// ===============================================================================
// APEX TITAN v112.0 (LEVIATHAN SUMMIT OVERLORD) - ULTIMATE ENGINE
// ===============================================================================
// MERGE SYNC: v111.0 (SUMMIT) + v21.1 (LEVIATHAN DUAL-SNIPE) + LOG DECODER
// ===============================================================================
...
// --- CONFIGURATION (v21.1 LEVIATHAN SUMMIT) ---
const GLOBAL_CONFIG = {
    ...
    // v21.1 STRATEGY SETTINGS
    MIN_WHALE_VALUE: 0.1,                // Heartbeat trigger for minor moves
    WHALE_MIN_ETH: 10.0,                 // Leviathan Log Threshold (Confirmed Volume)
    SUMMIT_WHALE_THRESHOLD: 15.0,        // Summit Tier trigger (Mempool Whale)
    GAS_LIMIT: 1200000n,                 
    MIN_NET_PROFIT: "0.01",              
    MIN_PROFIT_BUFFER: "0.005",          
    MARGIN_ETH: "0.012",                 // Leviathan specific safety gasket (v21.1)
    PRIORITY_BRIBE: 15n,                 
...
async function fetchAIAssessment(ethPrice) {
    const systemPrompt = "Professional cross-chain crypto analyst. Provide signal in JSON.";
    const userQuery = `ETH: $${ethPrice}. Suggest if Leviathan Summit strikes should be aggressive (BUY) or defensive (SELL).`;
    try {
...
╔════════════════════════════════════════════════════════╗
║   ⚡ APEX TITAN v112.0 | LEVIATHAN SUMMIT OVERLORD    ║
║   DUAL-SNIPE: PENDING TXS + SWAP LOG DECODING ACTIVE  ║
╚════════════════════════════════════════════════════════╝${TXT.reset}`);
...
async function initWorker(CHAIN) {
    ...
    async function safeConnect() {
        try {
            ...
            // v21.1 LEVIATHAN DUAL-SNIPE SCANNING
            if (DIVISION === 0 || DIVISION === 1) {
                // A. PENDING LAYER (Mempool Sniper)
                wsProvider.on("pending", async (txHash) => {
                    ...
                    if (tx && tx.to) {
                        ...
                        if (val >= GLOBAL_CONFIG.SUMMIT_WHALE_THRESHOLD) {
                             process.send({ type: 'WHALE_SIGNAL', chainId: CHAIN.chainId, target: tx.to, mode: "SUMMIT_PENDING" });
                        } else if (val >= GLOBAL_CONFIG.MIN_WHALE_VALUE && tx.to.toLowerCase() === CHAIN.uniswapRouter.toLowerCase()) {
                             process.send({ type: 'WHALE_SIGNAL', chainId: CHAIN.chainId, target: tx.to, mode: "STANDARD_PENDING" });
                        }
                    }
                });

                // B. LOG DECODER LAYER (Leviathan Sniper)
                ...
                wsProvider.on({ topics: [swapTopic] }, async (log) => {
                    try {
                        ...
                        if (maxSwap >= parseEther(GLOBAL_CONFIG.WHALE_MIN_ETH.toString())) {
...
async function strike(provider, wallet, iface, gasOracle, ethPrice, CHAIN, target, mode) {
    try {
        ...
        // Double-Gasket Profit Validation
        ...
        const marginWei = parseEther(GLOBAL_CONFIG.MARGIN_ETH); // v21.1 Floor
        ...
        if (rawProfit > (totalThreshold * aiMultiplier) / 100n) {
            ...
            if (mode === "LEVIATHAN_CONFIRMED") {
                console.log(`\n${TAG} ${TXT.yellow}${TXT.bold}🐳 LEVIATHAN VOLUME STRIKE CONFIRMED${TXT.reset}`);
            } else if (mode === "SUMMIT_PENDING") {
                console.log(`\n${TAG} ${TXT.red}${TXT.bold}🚨 SUMMIT WHALE STRIKE AUTHORIZED${TXT.reset}`);
            } else {
                console.log(`\n${TAG} ${TXT.green}💎 OPPORTUNITY AUTHORIZED${TXT.reset}`);
            }
            console.log(`   ↳ ⚖️ Leverage: ${formatEther(loanAmount)} ETH | Net: +${formatEther(netProfitEth)} ETH (~$${(parseFloat(formatEther(netProfitEth)) * ethPrice).toFixed(2)})${TXT.reset}`);
            ...
            console.log(`${TXT.green}${TXT.bold}🎉 STRIKE DISPATCHED: FUNDS AT ${GLOBAL_CONFIG.BENEFICIARY}${TXT.reset}`);
        } else {
